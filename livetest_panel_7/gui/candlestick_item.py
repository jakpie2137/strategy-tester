...
class CandlestickItem(pg.GraphicsObject):
    ...
    # maksymalny mnożnik odstępu względem typowej szerokości świecy (dla ograniczania "grubych" świec po lukach)
    MAX_SPACING_MULTIPLIER = 3.0
    ...
    def paint(self, p: QPainter, *args):
        ...
        ov = self.o[start:stop]
        cv = self.c[start:stop]
        lv = self.l[start:stop]
        hv = self.h[start:stop]
        ...
        if not np.any(non_empty):
            ...
            return
        uniq = uniq[non_empty]
        first_pos = first_pos[non_empty]
        last_pos = last_pos[non_empty]
    ...
    def _paint_direct(self, p: QPainter, start: int, stop: int, vb):
        ...
        if n == 1:
            ...
            # ograniczanie odstępów, żeby pojedyncze duże luki w danych
            # nie powodowały "przepuchnięcia" szerokości świec na całym widocznym zakresie.
            if n > 1:
                base = None
                try:
                    w_slice = self.w[start:stop]
                    if w_slice.size:
                        mask_w = np.isfinite(w_slice) & (w_slice > 0)
                        if np.any(mask_w):
                            base = float(np.nanmedian(w_slice[mask_w]))
                except Exception:
                    base = None
                if base is None or not np.isfinite(base) or base <= 0:
                    try:
                        mask_s = np.isfinite(spacing) & (spacing > 0)
                        if np.any(mask_s):
                            base = float(np.nanmedian(spacing[mask_s]))
                    except Exception:
                        base = None
                if base is None or not np.isfinite(base) or base <= 0:
                    base = 60.0
                max_spacing = float(base) * float(self.MAX_SPACING_MULTIPLIER)
                spacing = np.minimum(spacing, max_spacing)

        if vb is not None:
            dx, _ = vb.viewPixelSize()
            px_spacing = spacing / max(dx, 1e-12)
            px_body = np.clip(
                px_spacing * float(self.BODY_FRAC_OF_SPACING),
