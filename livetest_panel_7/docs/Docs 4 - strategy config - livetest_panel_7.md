...
- `r = ATR / price`
- `tp_long   = 1.0 + tp_k_long * r` (od entry)
- `sl_long   = 1.0 - sl_k_long * r` (od entry)
- `trail_long = 1.0 - ts_k_long * r` (od ts_benchmark, czyli lokalnego high/low (long/short) podczas pozycji po wybiciu TP)(jeśli trailing włączony)
...
- `type="fixed"` – decyzje są „schodkowe” (co bucket), 
- `type="rolling"` – wskaźniki reagują płynniej na każdą świecę. 

- TYPY BUCKETÓW - WAŻNE !! :
- `type`:
  - `"rolling"` – klasyczne „window” przesuwane co świecę, 
  - `"fixed"` – okna nieprzesuwne (np. co 5 świec powstaje nowa świeca „agregowana”). 
- `window`:
...
4. **Egzekucja i realizm:**
   ...
   - `CLOSE_EXECUTION_SLIPPAGE` + `ENTRY_EXECUTION_SLIPPAGE` slippage na wejściu i wyjściu (2 parametry),
