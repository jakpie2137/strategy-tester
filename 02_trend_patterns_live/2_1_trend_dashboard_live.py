# -*- coding: utf-8 -*-
"""
2_2_supp_resist_dashboard_live.py — Display-only dashboard for REALTIME DBs.

- Shows price from data/realtime_data.db (table candles).
- Shows trends from data/realtime_trend_detection.db (tables trends/trend_points).
- DOES NOT run detector automatically. Only on "Run detection".
- Auto-ensure: if trend tables don't exist, the dashboard will create them (empty) to avoid noisy errors.
- Optional: Auto refresh chart toggle (no detection).
"""
...
OUT_DB = os.path.join(HERE, "data", "realtime_trend_detection.db")
TABLE = "candles"

# Safe import of detector for button-triggered runs
spec = importlib.util.spec_from_file_location("trend_detection_live", os.path.join(HERE, "1_1_trend_detector_live.py"))
...
# Ensure trend tables exist to prevent "no such table" errors before first detection
try:
    trend_detection.ensure_output_db(OUT_DB)
except Exception as _e:
    log.warning("ensure_output_db on startup failed: %s", _e)
...
def read_trend_points(out_db: str, symbol: str, method: str, level: str):
    try:
        ...
        if "no such table" in msg:
            # Create tables and return empty
            try:
                trend_detection.ensure_output_db(out_db)
                log.info("read_trend_points: created missing tables in %s", out_db)
            except Exception as ee:
                log.error("read_trend_points: ensure_output_db failed: %s", ee)
...
def _level_keys():
    if hasattr(trend_detection, "ESTABLISH_LEN"):
        try: return list(getattr(trend_detection, "ESTABLISH_LEN").keys())
        except: pass
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Trend Dashboard LIVE (DISPLAY-ONLY)")
        ...
        # Auto refresh controls (chart only, no detection)
    ...
    def draw_plot(self):
        ...
        for (seg, side), grp in dft.groupby(["seg_index","side"]):
            ...
            lw = 1.6 if phase=="post" else 1.0
            tt = grp["idx"].to_numpy(); yy = grp["price"].to_numpy()
            order = np.argsort(tt)
            label = None
            if seg == 0:
                label = f"{side.replace('pre_','').replace('post_','')} ({phase})"
            self.ax.plot(tt[order], yy[order], linestyle=ls, linewidth=lw, alpha=1.0 if phase=="post" else 0.9, label=label)
